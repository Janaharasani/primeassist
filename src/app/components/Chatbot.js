    ...
    <div className={`w-[100%] h-[550px] relative top-6 mb-24 mx-auto rounded-lg shadow-2xl flex flex-col overflow-hidden transition-all duration-300 ${theme === 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-800'}`}>
          ...
          <div className="relative">
            ...
            {isMenuOpen && (
              <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} z-10`}>
                <a href="#" className="block px-4 py-2 text-sm hover:bg-blue-500 hover:text-white">Clear Conversation</a>
      ...
      <div className={`flex-1 overflow-y-auto p-4 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
        {messages.map((msg, index) => (
          <div key={index} className={`mb-4 ${msg.role === "user" ? "flex justify-end" : "flex justify-start"}`}>
            <div className={`flex max-w-[80%] ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${msg.role === "user" ? 'ml-3 bg-blue-500' : 'mr-3 bg-purple-500'} text-white`}>
        ...
        {isLoading && (
          <div className="flex justify-start mb-4">
            <div className="flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center mr-3 bg-purple-500 text-white">
              <RiRobot2Line />
            </div>
      ...
      <div className={`p-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
        <form onSubmit={handleSubmit} className="flex items-center">
          ...
          <button
            type="submit"
            className={`px-4 py-4 rounded-r-lg ${(!input.trim() || isLoading) ? (theme === 'dark' ? 'bg-gray-600 text-gray-400' : 'bg-gray-300 text-gray-500') : (theme === 'dark' ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white')} transition-colors`}
